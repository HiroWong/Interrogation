...
from baselines import PEZ, GBDA, ZeroShot, Int
...
def main():
    ...
    if mode == 'generate':
        ...
        elif baseline == 'Interrogation':
            # intervention_model = RoBERTaPredictor('hubert233/GPTFuzz', device='cuda:1') #finetuned for replying “Whether the following input is toxic or not: 1 for jailbreak,0 for reject”
            # selection_model = torch.hub.load('pytorch/fairseq', 'roberta.large.mnli') #finetuned for MNLI
            # selection_model.eval()
            # method = interrogation(judge_model, select_model, device=device)
            method_config = {
                "num_generate": 50,
                "batch_size": 10
            }
        ...
        if baseline in ['PEZ', 'GBDA']:
            ...
            model = AutoModelForCausalLM.from_pretrained(red_team_model_path, torch_dtype=torch.float16, device_map="cuda:0").eval()   
            use_fast_tokenizer = "LlamaForCausalLM" not in model.config.architectures
            tokenizer = AutoTokenizer.from_pretrained(red_team_model_path, padding_side='left', use_fast=use_fast_tokenizer)
            tokenizer.pad_token = tokenizer.eos_token
            # model = LLM(model=red_team_model_path,tokenizer=red_team_model_path,gpu_memory_utilization=0.7)
    ...
    if mode == 'evaluate':
        ...
        if not os.path.exists(generations_path):
            ...
            generations = generate_outputs(test_cases, model, tokenizer, batch_size=50, max_new_tokens=64, verbose=True)
